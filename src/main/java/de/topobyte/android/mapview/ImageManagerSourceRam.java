 ...
 * 
 */
public class ImageManagerSourceRam<T, D extends ReferenceCounted> extends
		AbstractImageManager<T, D> implements
		PriorityImageManager<T, D, Integer>
{
	...
	RefreshableCache<T, D> memoryCache = null;
	 ...
	 */
	public ImageManagerSourceRam(int nThreads, int desiredCacheSize,
			ImageSource<T, D> source)
	{
		...
		memoryCache = new ReferenceCountingCacheWrapper<>(memoryCache);
	...
	public D getButDoNotProduce(T thing)
	{
		return memoryCache.get(thing);
	}
